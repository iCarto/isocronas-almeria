...
if [[ -z "${ADMIN_PASSWORD}" ]]; then
    echo "Is mandatory provide a password for the 'admin' user"
    exit 1
fi
...
python back/manage.py shell -c "
from django.contrib.auth import get_user_model
user = get_user_model()._default_manager.db_manager('default').get(username='admin')
